...
async def dynamic_start(
    ...
    recipe_names = [r.strip() for r in recipes.split(",") if r.strip()]
    ...
    # Resolve recipe names → script sources via the same path /v1/recipes
    # exposes (built-ins + Medusa modules). Each recipe gets wrapped in
    # an IIFE so its top-level vars + helper functions don't collide
    # with the auto-hooks or other recipes loaded in the same session.
    if recipe_names:
        from mnexus.recipes import BUILTIN_RECIPES
        builtin_by_name = {r["name"]: r for r in BUILTIN_RECIPES}
        frida_engine = nexus.engines.get("frida")
        for name in recipe_names:
            if name in builtin_by_name:
                source = builtin_by_name[name]["script"]
            else:
                if frida_engine is None:
                    raise HTTPException(400, f"unknown recipe '{name}' — frida engine not registered to look up disk recipes")
                try:
                    source = frida_engine.load_medusa_module(name)  # type: ignore[attr-defined]
                except FileNotFoundError as exc:
                    raise HTTPException(400, f"unknown recipe '{name}': {exc}") from exc
            scripts.append((f"recipe::{name}", _wrap_in_iife(name, source)))
...
def _wrap_in_iife(recipe_name: str, source: str) -> str:
    ...
    Some Medusa modules declare classes like ``com.mnexus.NoopTrust``
    or top-level ``var CP = …`` that get duplicate-defined when N
    pinning bypass recipes are stacked. The IIFE plus a try/catch
    means a syntax error in one recipe still lets the rest of the
    session load.
    """
    safe_label = recipe_name.replace("`", "\\`").replace("$", "\\$")
    return (
        "(function () {\n"
        f"  try {{\n{source}\n  }} catch (e) {{\n"
        f"    try {{ send({{ channel: 'error', source: `{safe_label}`, "
        "description: '' + (e && (e.message || e)), stack: (e && e.stack) || '' }); } catch (_) {} }\n"
        "})();\n"
    )
