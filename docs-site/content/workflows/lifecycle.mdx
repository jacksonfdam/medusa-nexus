...
* Lists what gets wiped — workspace bytes, source artefact, secrets
  dir, reports, DB rows.
* Requires typing a confirmation phrase (``yes`` for single project,
  ``factory reset`` for the bulk wipe-all) before the destructive
  button enables.
* Renders the audit trail back on success — same JSON shape as the
  HTTP endpoint, just displayed instead of returned.
